...
def wmma(text):
    ...
    result=['v_mbcnt_lo_u32_b32 v92, -1, 0',
            'v_lshrrev_b32_e32 v92, 4, v92',
            'v_lshlrev_b32_e32 v92, 2, v92']
    for j in range(8):
        result.append(f'v_mov_b32_e32 v{84+j}, v{c+j}')
        for k in range(8):
            result += [f'ds_bpermute_b32 v93, v92, v{a+k} offset:{8*j}',
                       's_waitcnt lgkmcnt(0)',
                       f'v_dot2c_f32_f16 v{84+j}, v93, v{b+k}']
    result += [f'v_mov_b32_e32 v{d+j}, v{84+j}' for j in range(8)]
    return result

def main():
    probe = '--abi-probe' in sys.argv[1:]
    ...
    code += [f'.size {SYMBOL}, .-{SYMBOL}', '.section .rodata', '.p2align 6',
             ...
             '.end_amdhsa_kernel']
    notes=(ROOT/'analysis/gfx1100-notes.txt').read_text()
    ...
    subprocess.run([str(BIN/'ld.lld.exe'),'-shared',str(OUT/(stem+'.o')),'-o',str(OUT/(stem+'.co'))],check=True)
