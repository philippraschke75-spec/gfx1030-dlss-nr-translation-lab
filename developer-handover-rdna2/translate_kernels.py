...
def translate(name,lines,policy,md,ro,rofile,out,binpath,private_lds=False):
    ...
    if private and not private_lds:
    ...
    if ops & {'s_setpc_b64','s_swappc_b64'}:
        ...
        for i,(addr,text,raw) in enumerate(lines):
            if text.startswith('s_swappc_b64') and i>=2:
                # The source sequence may contain waits/barriers between the
                # PC setup and the indirect swap. Find the nearest setup in
                # the preceding instructions, then apply the signed low-word
                # delta to the address of S_GETPC_B64.
                for j in range(i-1, max(-1, i-24), -1):
                    if not lines[j][1].startswith('s_getpc_b64'):
                        continue
                    if j+1 >= i:
                        continue
                    add=lines[j+1][1]
                    m=re.search(r's_add_u32 s\d+, s\d+, (0x[0-9a-f]+)',add)
                    if not m:
                        continue
                    delta=int(m[1],16); delta-=0x100000000 if delta>=0x80000000 else 0
                    # GFX11 S_GETPC returns the address of the following
                    # instruction, so account for its four-byte encoding.
                    targets.append(hex(lines[j][0]+4+delta))
                    break
        raise ValueError('UNSUPPORTED: indirect helper relocation; resolved targets='+','.join(targets or ['unknown'])+'; helper=_Z10swin_layerR7SwinLDSPKhRK10BlobLayouti')
    ...
    private_stride=(private+15)//16*16
    private_start=(policy['group']+15)//16*16
    lds=private_start+private_stride*value(md,'max_flat_workgroup_size') if private else policy['group']
    ...
    for i,(addr,text,raw) in enumerate(lines):
        ...
        if op.startswith('v_wmma'):
            ...
            replacement=lower_private(text,scratch+12,scratch+13)
        elif op=='s_sendmsg_rtn_b64':
            raise ValueError('UNSUPPORTED: realtime message lowering needs clock contract')
    ...
    code += [f'.size {name}, .-{name}', '.section .rodata','.p2align 6',f'.amdhsa_kernel {name}',
             f'.amdhsa_group_segment_fixed_size {lds}', '.amdhsa_private_segment_fixed_size 0',
    ...
    code += ['.amdhsa_system_sgpr_private_segment_wavefront_offset 0',
    ...
    md=re.sub(r'\.private_segment_fixed_size:\s+\d+',f'.private_segment_fixed_size: 0',md)
    ...
    return dict(status='ASSEMBLED_UNVALIDATED',module=str(module),sha256=hashlib.sha256(module.read_bytes()).hexdigest(),
                source_vgprs=scratch,vgprs=vgprs,lds=lds,private_lds_bytes=private_stride*value(md,'max_flat_workgroup_size'),
                launch_contract='one-dimensional blocks; local y=z=1' if private else 'original launch contract',
...
def main():
    ...
    for name,offset in base.kd.find_kernel_kds(blob,sections,syms):
        ...
        row={'symbol':name}
        try:
            row.update(translate(name,dis[name],dec.dec(base.kd.dump_kd(blob,offset)),metadata(notes,name),ro,rofile,a.output,a.hip_bin,a.private_lds))
