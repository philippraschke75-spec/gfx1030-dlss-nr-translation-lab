...
int main(int argc, char** argv) {
    ...
    if(argc!=3) { std::puts("usage: final_head_test.exe module.co abi|constant|patterned|multigroup|basis"); return 1; }
    const bool abi=std::strcmp(argv[2],"abi")==0;
    const bool basis=std::strcmp(argv[2],"basis")==0;
    const bool multigroup=std::strcmp(argv[2],"multigroup")==0;
    const bool patterned=std::strcmp(argv[2],"patterned")==0 || multigroup;
    const unsigned groups=(multigroup || basis)?2:1;
    const size_t InputBytes=8192*groups,OutputBytes=16384*groups;
    if(!abi && !patterned && !basis && std::strcmp(argv[2],"constant")!=0) return 1;
    ...
    std::printf("HIP runtime=%d driver=%d; watchdog/TDR settings modified: NO\n",runtime,driver);
    ...
    if(basis) {
        // A is one-hot per row, with alternating +/-1; therefore A*B is
        // exactly one signed row of B. This reference needs no WMMA model,
        // lookup-table decoder or floating-point rounding implementation.
        // Every finite E4M3 byte occurs in B, including both zero encodings.
        std::fill(in.begin()+Guard,in.end()-Guard,0);
        std::vector<bool> seen_input(InputBytes),seen_weights(WeightBytes),seen_output(OutputBytes);
        bool codes[256]{};
        auto weight_code=[](int k,int col) -> unsigned char {
            int n=(k*37+col*17)%254;
            return n<127?n:(0x80|(n-127));
        };
        for(unsigned r=0;r<16*groups;++r) for(int k=0;k<512;++k) {
            ...
            seen_input[off]=true;
            if(k==int((r*17)%512)) in[Guard+off]=(r&1)?0xb8:0x38;
        }
        for(int k=0;k<512;++k) for(int col=0;col<1024;++col) {
            size_t off=weight_offset(k,col);
            if(off>=WeightBytes || seen_weights[off]) return 7;
            seen_weights[off]=true;
            auto bits=weight_code(k,col);
            weights[Guard+off]=bits;codes[bits]=true;
        }
        for(unsigned r=0;r<16*groups;++r) for(int col=0;col<1024;++col) {
            ...
            seen_output[off]=true;
            unsigned char bits=weight_code((r*17)%512,col);
            // Accumulation starts at +0: either signed zero contributes +0.
            expected[off]=(bits&0x7f)?(bits^((r&1)?0x80:0)):0;
        }
        int covered=0; for(bool present:codes) covered+=present;
        if(covered!=254 || codes[0x7f] || codes[0xff]) return 8;
        std::printf("Basis reference: signed row selection, 32 distinct reduction indices, %d finite FP8 weight encodings\n",covered);
    }
    if(patterned) {
    ...
    if(abi) {
        ...
        FILE* f=nullptr;
        const char* output_name=basis?"final-head-basis-output.bin":(multigroup?"final-head-multigroup-output.bin":
    ...
    std::printf("%s: %s\n",abi?"ABI PROBE":(basis?"BASIS KERNEL TEST":(patterned?"PATTERNED KERNEL TEST":"CONSTANT KERNEL TEST")),pass?"PASS":"FAIL");
    return pass?0:3;
}
