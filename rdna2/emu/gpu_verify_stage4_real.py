...
cur = pooled2
pooled3 = None
CKPT3 = D.ROOT / 'build' / 'pooled3_checkpoint.npy'
if CKPT3.exists():
    pooled3 = np.load(CKPT3)
    print('loaded pooled3 checkpoint from disk, skipping stage 3 recompute, %.0fs' % (time.time() - t_start), flush=True)
else:
    start_i = 9
    for i in range(13, 8, -1):   # deliberately excludes block14: its checkpoint holds the regular output, not the
        ck = D.ROOT / 'build' / ('block%d_checkpoint.npy' % i)  # pooled one pooled3 needs, so block14 must always re-run here
        if ck.exists():
    ...
    np.save(CKPT3, pooled3)
    print('saved pooled3 checkpoint to disk for future runs', flush=True)

print('real stage-3 pooled data ready, %.0fs; now GPU-verifying stage 4 (blocks 15-22)' % (time.time() - t_start), flush=True)

# ---- stage 4: blocks 15-22, C=256, H=W=N/8, each GPU-verified ----
EIGHTH = N // 8
# per VARPARAMS_HOST_CONTRACT.md: 8-block stage repeats modes 0,1,2,3,0,1,2,3; first (block15) flags=1, last (block22) flags=4
SCHEDULE4 = [('block15.bin', 1, 0, 0), ('block16.bin', 0, -4, -4), ('block17.bin', 0, -4, 0), ('block18.bin', 0, 0, -4),
...
for i in range(22, 14, -1):
    ...
    if ck.exists():
        cur = np.load(ck); start_i = i + 1
        print('resuming stage 4 from block%d checkpoint, %.0fs' % (i, time.time() - t_start), flush=True)
        break
pooled4 = None
for i, (wf, flags, ox, oy) in enumerate(SCHEDULE4, start=15):
    if i < start_i:
        continue
    w = np.frombuffer((WDIR / wf).read_bytes(), np.uint8)
    cur_local = cur
    def patch_b(a, w=w, cur_local=cur_local):
        a[0 * V.SLOT:0 * V.SLOT + len(cur_local)] = cur_local
        a[2 * V.SLOT:2 * V.SLOT + len(w)] = w
    ref_b, init_b = gpu_verify(ENC256_SYM, ENC256_LDS, EIGHTH, EIGHTH, flags, ox, oy, patch_b, 'block%d' % i)
    cur = ref_b[1 * V.SLOT:2 * V.SLOT].copy()
    np.save(D.ROOT / 'build' / ('block%d_checkpoint.npy' % i), cur)
    if flags & 4:
        pool_seg = ref_b[POOL_SLOT * V.SLOT:(POOL_SLOT + 1) * V.SLOT]
