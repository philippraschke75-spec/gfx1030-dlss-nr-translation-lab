...
for i, (wfile, flags, (ox, oy)) in enumerate(SCHEDULE, start=1):
    ...
    if flags & 4:
        pooled = ref_b[POOL_SLOT * V.SLOT:(POOL_SLOT + 1) * V.SLOT].copy()

# ---- stage-2 encoder blocks 5-8 (k_swin_var<64,false>, C=64, H=W=N/2), fed by block4's fused pooled output ----
HALF = N // 2
SCHEDULE2 = [
    ('block5.bin', 1, (0, 0)),
    ('block6.bin', 0, (-4, -4)),
...
cur = pooled
for i, (wfile, flags, (ox, oy)) in enumerate(SCHEDULE2, start=5):
    w = np.frombuffer((WEIGHTS_DIR / wfile).read_bytes(), np.uint8)
    grid_b = ((HALF + 7) // 8, (HALF + 7) // 8)
    cur_local = cur

    def kernarg_b(flags=flags, ox=ox, oy=oy, grid_b=grid_b):
        return V.make_kernarg(H=HALF, W=HALF, offy=ox, offx=oy, flags=flags, grid=grid_b)

    def patch_b(a, w=w, cur_local=cur_local):
        a[0 * V.SLOT:0 * V.SLOT + len(cur_local)] = cur_local
        a[2 * V.SLOT:2 * V.SLOT + len(w)] = w

    ref_b = run_and_verify(ENC64_SYM, ENC64_LDS, kernarg_b, patch_b, grid_b, 'block%d' % i)
    cur = ref_b[1 * V.SLOT:2 * V.SLOT].copy()
