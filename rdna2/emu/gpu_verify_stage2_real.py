...
# ---- rebuild real pooled data (emulator only; already GPU-verified upstream) ----
...
print('real stage-1 pooled data ready, %.0fs; now GPU-verifying stage 2 (blocks 5-8)' % (time.time() - t_start), flush=True)

# ---- stage 2: blocks 5-8, C=64, H=W=N/2, each GPU-verified ----
HALF = N // 2
...
for i, (wf, flags, ox, oy) in enumerate(SCHEDULE2, start=5):
    ...
    def patch_b(a, w=w, cur_local=cur_local):
        a[0 * V.SLOT:0 * V.SLOT + len(cur_local)] = cur_local
        a[2 * V.SLOT:2 * V.SLOT + len(w)] = w
    if i < 8:   # blocks 5-7 already GPU-verified individually; emulator-only here to reach block8 faster
        ref_b = run_local(ENC64_SYM, ENC64_LDS, HALF, HALF, flags, ox, oy, patch_b, 'block%d(emu-only, already GPU-verified)' % i)
        init_b = None
    else:
        ref_b, init_b = gpu_verify(ENC64_SYM, ENC64_LDS, HALF, HALF, flags, ox, oy, patch_b, 'block%d' % i)
    cur = ref_b[1 * V.SLOT:2 * V.SLOT].copy()
    if flags & 4 and init_b is not None:
