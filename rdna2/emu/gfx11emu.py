...
SALU2 = {
    ...
    's_add_i32': lambda x, y, c: ((x + y) & M32, None),
...
class Exec:
    ...
    def _decode(s, addr, op, args, size):
        ...
        if (m := re.fullmatch(r's_cmpk?_(eq|lg|gt|ge|lt|le)_(i|u)32', base)):
            c, ty = m[1], m[2]
            ...
            if two:
                for j, oo in ((0, o0), (1, o1)):
                    ...
                    if ld:
                        ...
                        w.V[vd['i'] + j, idx] = v if v.ndim == 1 else v[:, 0]
                    else:
    ...
    def _valu(s, b, P, mods):
        ...
        if b == 'v_pk_mul_f16':
            def pk(w):
                x, y = w.r32(P[1]), w.r32(P[2])
                lo = hbits(h(x).view(np.float16) * h(y).view(np.float16))
                hi = hbits((x >> np.uint32(16)).astype(np.uint16).view(np.float16) * (y >> np.uint32(16)).astype(np.uint16).view(np.float16))
                w.wv(P[0], lo | (hi << np.uint32(16)))
            return pk
...
def run_workgroup(prog, gmem, lds_size, nthreads, init_sgprs, max_steps=200_000_000, trace=None):
    ex = Exec(prog)
