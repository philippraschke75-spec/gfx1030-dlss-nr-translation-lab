...
typedef int (WINAPI *hipModuleGetFunction_fn)(hipModule_t, const char*, hipFunction_t*);
...
static bool V5LocateOffloadBundle(HMODULE module, V5BundleInfo* outBundle)
{
    ...
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; ++i) {
        ...
        const uintptr_t secStartVa = reinterpret_cast<uintptr_t>(module) + section.VirtualAddress;
        const size_t secLen = section.Misc.VirtualSize ? section.Misc.VirtualSize : section.SizeOfRawData;
        const uint8_t* secStart = reinterpret_cast<const uint8_t*>(secStartVa);
        const uint8_t* secEnd = secStart + secLen;

        for (const uint8_t* p = secStart; p + 32 < secEnd; ++p) {
            if (memcmp(p, V5_OFFLOAD_MARKER, strlen(V5_OFFLOAD_MARKER)) == 0) {
                bundleMarker = p;
    ...
    outBundle->bundleSize = bundleSectionLen - static_cast<size_t>(bundleMarker - bundleSectionStart);
    outBundle->headerSize = 4096;

    V5Log("[DLSSNR-V5] BUNDLE", "START=0x%p SIZE=0x%zx HEADER_SIZE=0x%zx", outBundle->bundleStart, outBundle->bundleSize, outBundle->headerSize);

    const size_t scanWindow = std::min<size_t>(outBundle->bundleSize, 0x20000U);
    const uint8_t* bundleEnd = outBundle->bundleStart + scanWindow;

    std::vector<V5BundleTarget> foundTargets;
    const uint8_t* start = outBundle->bundleStart + strlen(V5_OFFLOAD_MARKER);
    const uint8_t* end =
        (bundleEnd < outBundle->bundleStart + outBundle->bundleSize)
            ? bundleEnd
            : outBundle->bundleStart + outBundle->bundleSize;

    for (const uint8_t* p = start; p + 8 < end; ) {
        if (*p == '\0') {
            ++p;
            continue;
        }

        const size_t len = V5StrnLen(reinterpret_cast<const char*>(p), static_cast<size_t>(end - p));
        if (len > 3 && len < 512) {
            std::string candidate(reinterpret_cast<const char*>(p), len);
            if (candidate.find("x86_64") != std::string::npos ||
                candidate.find("amdgcn") != std::string::npos ||
                candidate.find("gfx") != std::string::npos ||
                candidate.find("host") != std::string::npos) {
                foundTargets.push_back({ candidate, static_cast<uintptr_t>(p - bundleMarker), len });
            }
            p += len + 1;
            continue;
        }
    ...
    if (foundTargets.empty()) {
        V5Log("[DLSSNR-V5] ERROR", "TARGET_LIST_EMPTY");
        return false;
    }

    outBundle->targets = foundTargets;
    outBundle->targetCount = foundTargets.size();
    V5Log("[DLSSNR-V5] BUNDLE", "TARGET_COUNT=%zu", outBundle->targetCount);
    for (const auto& target : outBundle->targets) {
        V5Log("[DLSSNR-V5] TARGET", "%s", target.name.c_str());
    }

    const auto targetIt = std::find_if(foundTargets.begin(), foundTargets.end(), [](const V5BundleTarget& item) {
        ...
        return lower.find("gfx1032") != std::string::npos;
    });

    if (targetIt == foundTargets.end()) {
        V5Log("[DLSSNR-V5] ERROR", "GFX1032_TARGET_NOT_FOUND");
        return false;
    }

    const uint8_t* targetLoc = bundleMarker + targetIt->start;
    const uint8_t* searchStart = targetLoc;
    const uint8_t* searchEnd = bundleMarker + std::min<size_t>(outBundle->bundleSize, 0x50000U);
    uintptr_t payloadOffset = 0;
    size_t payloadSize = 0;

    for (const uint8_t* q = searchStart; q + 16 < searchEnd; q += 8) {
        const uint64_t off = V5ReadU64LE(q);
        const uint64_t sz = V5ReadU64LE(q + 8);
        if (off >= 0x1000 && off < 0x1000000 && sz >= 0x1000 && sz < 0x1000000) {
            payloadOffset = static_cast<uintptr_t>(off);
            payloadSize = static_cast<size_t>(sz);
            break;
        }
    }

    if (payloadOffset == 0 || payloadSize == 0) {
        V5Log("[DLSSNR-V5] ERROR", "GFX1032_PAYLOAD_METADATA_INVALID");
        return false;
    }

    const uintptr_t payloadStartVa = reinterpret_cast<uintptr_t>(module) + payloadOffset;
    const uintptr_t payloadEndVa = payloadStartVa + payloadSize;
    V5Log("[DLSSNR-V5] ELF", "START=0x%llx SIZE=0x%zx END=0x%llx", static_cast<unsigned long long>(payloadStartVa), payloadSize, static_cast<unsigned long long>(payloadEndVa));

    // Require the payload to live inside one of the loaded PE section ranges.
    bool isInRange = false;
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; ++i) {
        auto& section = sections[i];
        const uintptr_t secStart = reinterpret_cast<uintptr_t>(module) + section.VirtualAddress;
        const uintptr_t secEnd = secStart + (section.Misc.VirtualSize ? section.Misc.VirtualSize : section.SizeOfRawData);
        if (payloadStartVa >= secStart && payloadEndVa <= secEnd) {
            isInRange = true;
            break;
        }
    }
    ...
    outBundle->elfPointer = reinterpret_cast<const uint8_t*>(payloadStartVa);
    outBundle->elfSize = payloadSize;
    outBundle->elfVa = payloadStartVa;
    outBundle->elfRva = payloadOffset;
    return true;
}
